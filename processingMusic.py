...
def process(audio_path, file, folder):
    # Carregar o arquivo de áudio
    #audio_path = "music.wav"  # Caminho do arquivo
    music, sr = librosa.load(audio_path, sr=16000)  # sr=None mantém a taxa de amostragem original, no caso fazendo um downsampling aqui.

    duration = len(music) / sr

    # Filtragem (removendo frequências baixas que podem ser ruídos)
    cutoff = 50 # Remove frequências abaixo dessa
    order=5
    nyquist = 0.5 * sr
    ...
    frequencies_fft = np.fft.fftfreq(len(magnitude_fft),1/sr)  # Calcula os valores de frequência
    ...
    # Resumindo a transformada de fourier
    ...
    frame_length = int(0.02 * sr)  # 20ms em amostras
    ...
    mel_spectrogram = librosa.feature.melspectrogram(y=music, sr=sr, n_fft=frame_length, hop_length=hop_length, n_mels=n_mels)
    ...
    cqt = librosa.cqt(music, sr=sr, hop_length=hop_length)
    ...
    tempo, beat_dense = librosa.beat.beat_track(y=music, sr=sr)
    # Converter os índices das batidas para tempos em segundos
    beat_times = librosa.frames_to_time(beat_dense, sr=sr)
    ...
    bps = np.size(times[beat_dense])/(duration)
        ...
        if os.path.isdir(folder_path):  # Verifica se é uma pasta
            for file in os.listdir(folder_path):
                if file.endswith(('.wav')):  # Adicione outros formatos de áudio, se necessário
                    ...
                    process(audio_path, file, folder)
