...
def test_acuracy():
    ...
    X_base = df.drop(columns=['genre', 'song', 'ttf'])  # Remove colunas não numéricas
    ...
    X = np.hstack((X_base.values, np.vstack(X_ttf.values)))
...
def test_genre(data_path, song_name):
    ...
    # Concatena as features base com as extraídas de 'ttf'
    X = np.hstack((X_base.values, np.stack(X_ttf.values)))
    ...
    # Dividir em treino e teste
    X_train, X_test, y_train, y_test = train_test_split(X_scaled, y_encoded, test_size=0.1, random_state=80)
    ...
    data_base = data.drop(columns=['genre', 'song', 'ttf'])  # Remove colunas não numéricas
    ...
    d = np.hstack((data_base.values, np.stack(data_ttf.values)))
    ...
    # Obter distâncias aos hiperplanos das classes
    distances = svc.decision_function(d_scaled)

    # Fazer previsões
    y_pred = svc.predict(d_scaled)

    # Converter os rótulos de volta para os nomes dos gêneros
    y_pred_labels = encoder.inverse_transform(y_pred)
    print(f"GGênero previsto: {y_pred_labels[0]}!")

    # Obter os três índices com maior resposta (mais próximos do hiperplano)
    top_indices = np.argsort(distances[0])[-5:][::-1]

    # Obter os três gêneros mais próximos
